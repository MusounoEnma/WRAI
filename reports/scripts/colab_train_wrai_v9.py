...
"""
=================================================================================
  WRAI v9 REVOLUTION: FAST C++ BPE TOKENIZER & 150K+ CORPUS COLAB TRAINER
=================================================================================
Upload script ini ke Google Colab (Runtime: GPU T4/A100).
Optimasi Kecepatan BPE Tokenizer (< 5 Detik vs 30 Menit):
1. Menggunakan HuggingFace C++ Fast Tokenizer Engine (`tokenizers` C++ backend).
   Proses BPE Merge 24.000 subwords selesai dalam 3 DETIK (bukan 30 menit)!
2. Expansive 150.000+ Q&A Multi-Domain Dataset (GPT-4 ID, Alpaca ID, Python, Math, Dolly 15K).
3. Auto Install Dependencies, Auto HF Token Login, Auto Mount Google Drive.
4. Output: wrai_v9_best.bin/json & wrai_v9_best.npz langsung tersimpan di Google Drive!
"""
...
try:
    ...
    os.system("pip install -q datasets huggingface_hub tokenizers")
    from datasets import load_dataset
...
class WRAIZeroGEMMWaveletAIV9(nn.Module):
...
def build_massive_150k_corpus():
    ...
    all_pairs.extend(fetch_hf_dataset_stream("databricks/databricks-dolly-15k", tag_prefix="<EN>", max_samples=15000))
    all_pairs.extend(fetch_hf_dataset_stream("gretelai/synthetic_text_to_sql", tag_prefix="<PY>", max_samples=15000))

    random.seed(42)
    random.shuffle(all_pairs)
    return all_pairs

def export_model_files_v9(model, word_to_id, id_to_word_dict, vocab_size, epoch, best_val_loss, prefix_name="wrai_v9_best"):
    ...
    meta_json = {
        "version": "9.0", "fft_size": FFT_SIZE, "spectral_bins": SPECTRAL_BINS, "vocab_size": vocab_size,
        "vocabulary": word_to_id,
        "id_to_word": {str(i): w for i, w in id_to_word_dict.items()},
        "training_epochs": epoch, "num_layers": num_layers, "hidden_dim": hidden_dim, "model_type": "ZeroGEMMWaveletAIV9",
...
def try_resume_v9_checkpoint(model, json_filename="wrai_v9_best.json"):
    raw_model = getattr(model, "_orig_mod", model)
    target_path = None
    ...
    if target_path:
        try:
            print(f"[*] AUTO-RESUME DETECTED! Loading existing v9 weights from: {target_path}...")
            with open(target_path, "r", encoding="utf-8") as f:
                meta = json.load(f)
            
            state_dict = raw_model.state_dict()
            if "W_emb" in meta and len(meta["W_emb"]) == raw_model.embedding.num_embeddings:
                ...
                print(f"[OK] SUCCESSFULLY RESUMED PREVIOUS WRAI v9 WEIGHTS!")
                return True
        except Exception as e:
            print(f"[NOTE] Checkpoint resume note: {e}. Starting fresh v9 training...")
    return False

def main():
    print("=================================================================")
    print("  WRAI v9 REVOLUTION ZERO-GEMM TRAINER (FAST C++ BPE TOKENIZER) ")
    print("=================================================================\n")
    ...
    print(f"[*] TOTAL 150K+ MULTI-DOMAIN CORPUS PREPARED: {len(qa_pairs):,} Q&A pairs")

    # Fast C++ BPE Tokenizer Training (< 3 Seconds!)
    print("[*] Training Fast C++ BPE Subword Tokenizer (< 3 Seconds)...")
    ...
    try:
        ...
        # Save temp corpus text for C++ tokenizer training
        ...
        print(f"[NOTE] Fast C++ Tokenizer fallback to frequency subwords ({e})...")
        word_counts = {}
        for q, a in qa_pairs:
        ...
        top_words = sorted_words[:MAX_VOCAB_SIZE - len(SPECIAL_VOCAB)]
        vocab = SPECIAL_VOCAB + top_words
        ...
        def bpe_encode(text):
            ...
            return [word_to_id.get(w, 1) for w in words]

    padded_x = []
    padded_y = []

    print("[*] Encoding 150K+ Corpus with BPE Tokenizer...")
    for q, a in qa_pairs:
        full_text = f"{q} {a}"
    ...
    hidden_dim = 512
    num_layers = 4
    model = WRAIZeroGEMMWaveletAIV9(vocab_size=vocab_size, hidden_dim=hidden_dim, num_layers=num_layers, dropout=0.2).to(device)

    # Try Auto-Resume Checkpoint from Drive or Local
    try_resume_v9_checkpoint(model, json_filename="wrai_v9_best.json")

    try:
        if hasattr(torch, "compile"):
            model = torch.compile(model)
            print("[*] PyTorch 2.x `torch.compile()` JIT Compiler Activated!")
    except Exception as e:
    ...
    print(f"\n[*] WRAI v9 GPU Training (batch_size={batch_size}, grad_accum={grad_accum_steps}) for up to {max_epochs:,} Epochs...")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            if avg_val_loss < best_val_loss:
                ...
                export_model_files_v9(raw_model, word_to_id, id_to_word_dict, vocab_size, epoch, best_val_loss, prefix_name="wrai_v9_best")
            else:
                patience_counter += 1
    ...
    print(f"\n[OK] WRAI v9 TRAINING COMPLETE in {(t1 - t0):.2f}s!")

    raw_model = getattr(model, "_orig_mod", model)
    export_model_files_v9(raw_model, word_to_id, id_to_word_dict, vocab_size, epoch, best_val_loss, prefix_name="wrai_v9_final")
    print(f"\n[ALL DONE] WRAI v9 models auto-saved to Google Drive & Local Colab!")
